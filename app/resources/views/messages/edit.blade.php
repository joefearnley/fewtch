<x-layouts.app>
    <div class="max-w-5xl mx-auto py-10 px-4">
        <h1 class="text-3xl font-bold mb-6 text-gray-900 dark:text-gray-100">{{ __('Edit Message') }}</h1>
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm p-6 border border-gray-200 dark:border-gray-700">
            <form action="{{ route('messages.update', $message) }}" method="POST" class="space-y-3">
                @csrf
                @method('PATCH')
                <div class="mb-4">
                    <x-forms.input label="Subject" labelClass="sr-only" name="subject" type="subject" placeholder="Add your message subject here..." value="{{ $message->subject }}" />
                </div>
                <div>
                <textarea
                    ...
                    id="content"
                    rows="10"
                    placeholder="Compose your message here...">{{ $message->content }}</textarea>
                @error('content')
                    <div class="text-red-700 dark:text-red-400 text-sm mb-4">{{ $errorMessage }}</div>
                @enderror
                </div>
                    ...
                    <div>
                        <div class="flex flex-col md:flex-row md:items-start">
                            ...
                            <div class="mr-6 text-left">
                                ...
                                <input type="date" id="send_date" placeholder="" name="send_date" class="w-full px-4 py-1.5 rounded-lg text-gray-700 dark:text-gray-300 bg-gray-50 dark:bg-gray-700 border border-gray-300 dark:border-gray-600 focus:outline-none focus:ring-2 focus:ring-sky-500 focus:border-transparent" value="{{ $message->send_date }}">
                            </div>
                        </div>
                        @error('send_date')
